...
import com.jkys.phobos.util.ByteUtil;
...
public class Response {
    ...
    static public byte[] toBytes(Response response, byte type) throws Exception{
        ...
        if(response.isSuccess()){
            ...
            byte[] errMessageLen = ByteUtil.shortToBytes((short)errMessage.length);
    ...
    static public Response toResponse(byte[] bytes) throws Exception{
        ...
        if(success){
            ...
            short errMessageLen = ByteUtil.bytesToShort(Arrays.copyOfRange(bytes, index, index + 2));
            index = index + 2;
